...
RSpec.describe 'invoices show' do
  ...
  it "shows a select field to update the invoice status" do
    ...
    it 'should show merchants when no bulk discount is eligble' do
      ...
      discount_a = BulkDiscount.create!(percentage_discount: 10, quantity:  15, merchant_id: merchant1.id)

      visit merchant_invoice_path(merchant1, invoice_1)

        expect(page).to have_content("Total Revenue: $150")
        expect(page).to have_content("Total Discount: $0")
        expect(page).to have_content("Net Revenue: $150")
    end

    it 'should have a link to the bulk discount that was applied' do
      ...
      visit merchant_invoice_path(@merchant1, @invoice_1)
        ...
        within "#the-status-#{@ii_11.id}" do
          ...
          expect(current_path).to eq(merchant_bulk_discount_path(@merchant1, discount_a))
    end
  end
