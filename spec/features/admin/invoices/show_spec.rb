...
describe 'Admin Invoices Index Page' do
  before :each do
    ...
    @discount1 = BulkDiscount.create!(name: 'Cheap Things', percentage: 20, quantity_threshold: 10, merchant_id: @m1.id)
    @discount2 = BulkDiscount.create!(name: 'Mythical Deals', percentage: 30, quantity_threshold:15, merchant_id: @m1.id)
    @discount3 = BulkDiscount.create!(name: 'Surplus Value', percentage: 40, quantity_threshold: 20, merchant_id: @m1.id)
  ...
  describe 'user story 8' do
    ...
    it "shows the total discounted revenue from this invoice which includes bulk discounts in the calculation" do
      ...
      expect(page).to have_content("Total Discount Revenue: $#{@i1.total_discount_revenue}")
      expect(page).to_not have_content(@i2.total_discount_revenue)
    end
  end
end
