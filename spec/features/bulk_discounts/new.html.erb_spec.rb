...
describe 'merchant bulk discount new page' do
  ...
  it 'has a form to create a new discount, then takes you back to the index page' do
    fill_in('Percentage', with: 80)
    fill_in('Threshold', with: 40)
    click_button('Submit')
    expect(current_path).to eq("/merchant/#{@merchant1.id}/bulk_discounts")
    expect(page).to have_content('80 percent off of 40 items')
  end
end
